...
router.get("/me", async (req, res) => {
  ...
  if (token) {
    try {
      const { rows } = await db.query(
        `SELECT u.id, u.username, u.color_bg, u.color_fg FROM users u
      ...
      if (rows.length) {
        const u = rows[0];
        return res.json({ id: u.id, username: u.username, color: { bg: u.color_bg, fg: u.color_fg }, token });
      }
    } catch(err) { console.error("token auth:", err.message); }
  }
